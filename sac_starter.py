...
class SACAgent:
    ...
    def update_model(self) -> Tuple[torch.Tensor, ...]:
        ...
        #target V value from the frozen target network
        next_v = self.vf_target(next_state)         

        #mask out terminal states (done=1 → no future reward)
        next_v = next_v * mask                     

        #Bellman target y = r + γ * V_target(s')
        q_target = reward + self.gamma * next_v      

        #no gradient through the target 
        q_target = q_target.detach()                 

        #Q estimates from both networks for s, a
        qf_1_pred = self.qf_1(state, action)      
        qf_2_pred = self.qf_2(state, action)       

        #cMSE loss for each Q network
        ...
        q_pred = torch.min(
            self.qf_1(state, new_action),   # self.qf_1 not qf_1
            self.qf_2(state, new_action)    # self.qf_2 not qf_2
        )
    ...
    def save_checkpoint(self, checkpoint_dir: str, step: int):
        ...
        path = os.path.join(checkpoint_dir, f"checkpoint_step{step}.pth")
        torch.save(
            {
                "step": step,
